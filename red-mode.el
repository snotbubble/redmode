...
;; faces for this mode - cpb 210321

(defvar red-control-word 'red-control-word)
(defface red-control-word '((t (:foreground "red1" :weight bold)))
	"control"
	:group 'red-mode)
(defvar red-specialstring-word 'red-specialstring-word)
(defface red-specialstring-word '((t (:foreground "red2" :weight bold)))
	"specialstring"
	:group 'red-mode)
(defvar red-context-word 'red-context-word)
(defface red-context-word '((t (:foreground "DodgerBlue" :weight bold)))
	"context"
	:group 'red-mode)
(defvar red-comparison-word 'red-comparison-word)
(defface red-comparison-word '((t (:foreground "IndianRed" :weight bold)))
	"comparison"
	:group 'red-mode)
(defvar red-logic-word 'red-logic-word)
(defface red-logic-word '((t (:foreground "tomato" :weight bold)))
	"logic"
	:group 'red-mode)
(defvar red-series-word 'red-series-word)
(defface red-series-word '((t (:foreground "coral" :weight bold)))
	"series"
	:group 'red-mode)
(defvar red-viewcmd-word 'red-viewcmd-word)
(defface red-viewcmd-word '((t (:foreground "CornflowerBlue" :weight bold)))
	"viewcmd"
	:group 'red-mode)
(defvar red-drawcmd-word 'red-drawcmd-word)
(defface red-drawcmd-word '((t (:foreground "CornflowerBlue" :weight bold)))
	"drawcmd"
	:group 'red-mode)
(defvar red-io-word 'red-io-word)
(defface red-io-word '((t (:foreground "goldenrod" :weight bold)))
	"io"
	:group 'red-mode)
(defvar red-events-word 'red-events-word)
(defface red-events-word '((t (:foreground "orange" :weight bold)))
	"events"
	:group 'red-mode)
(defvar red-system-word 'red-system-word)
(defface red-system-word '((t (:foreground "yellow" :weight bold)))
	"system"
	:group 'red-mode)
(defvar red-datetime-word 'red-datetime-word)
(defface red-datetime-word '((t (:foreground "OliveDrab" :weight bold)))
	"datetime"
	:group 'red-mode)
(defvar red-help-word 'red-help-word)
(defface red-help-word '((t (:foreground "LimeGreen" :weight bold)))
	"help"
	:group 'red-mode)
(defvar red-viewprp-word 'red-viewprp-word)
(defface red-viewprp-word '((t (:foreground "LightSeaGreen" :weight bold)))
	"viewprp"
	:group 'red-mode)
(defvar red-drawprp-word 'red-drawprp-word)
(defface red-drawprp-word '((t (:foreground "LightSeaGreen" :weight bold)))
	"drawprp"
	:group 'red-mode)
(defvar red-datatypes-word 'red-datatypes-word)
(defface red-datatypes-word '((t (:foreground "MediumPurple" :weight bold)))
	"datatypes"
	:group 'red-mode)
(defvar red-viewobj-word 'red-viewobj-word)
(defface red-viewobj-word '((t (:foreground "DodgerBlue" :weight bold)))
	"viewobj"
	:group 'red-mode)
(defvar red-drawobj-word 'red-drawobj-word)
(defface red-drawobj-word '((t (:foreground "DodgerBlue" :weight bold)))
	"drawobj"
	:group 'red-mode)
(defvar red-constants-word 'red-constants-word)
(defface red-constants-word '((t (:foreground "plum" :weight bold)))
	"constants"
	:group 'red-mode)
(defvar red-math-word 'red-math-word)
(defface red-math-word '((t (:foreground "HotPink" :weight bold)))
	"math"
	:group 'red-mode)
(defvar red-modifiers-word 'red-modifiers-word)
(defface red-modifiers-word '((t (:foreground "red3" :weight bold)))
	"modifiers"
	:group 'red-mode)
(defvar red-comment-word 'red-comment-word)
(defface red-comment-word '((t (:foreground "ForestGreen" :weight bold)))
	"modifiers"
	:group 'red-mode)
...
(
	setq red-font-lock-keywords (
		let* (
			...
			(x-io '("ask" "change-dir" "close" "confirm" "connected?" "delete" "dispatch" "echo" "exists?" "get-modes" "info?" "input" "input?" "list-dir" "make-dir" "modified?" "open"  "query" "read" "read-io" "rename" "resend" "save" "script?" "secure" "send" "set-modes" "set-net" "size?" "to-local-file" "to-rebol-file" "update" "wait" "what-dir" "write-io" "write" ))
